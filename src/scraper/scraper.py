...
# Global article counter and lock to enforce max article limit across all concurrent tasks.
# Reset when the scraping completes.
articles_counter = 0
...
async def scrape_article(
    ...
    Args:
        ...
        semaphore: asyncio.Semaphore for rate-limiting concurrent requests.
        limit: Maximum number of articles to scrape.

    Returns:
        An Article object containing structured content and metadata,
        or None if the article cannot be retrieved or parsed or max article limit is reached.
    ...
    Notes:
        ...
        - The function also checks a global article counter to enforce a maximum article limit across the entire scraping session.
    """
        ...
        async with counter_lock:
            ...
            for node in article.children:
                ...
                # Extract paragraphs (main article content)
                    ...
                    if text:
                        article_contents.append(
                            ArticleContent(type="paragraph", text=text)
                        )
                ...
                # Extract lists items
                    ...
                    article_contents.append(
                        ArticleContent(
                            type="list", ordered=node.name == "ol", items=items
                        )
                    )

            word_count = sum(
                len(block.text.split()) if block.text else 0
...
async def scrape_category(
    ...
    for link in article_links:
        ...
        tasks.append(
            scrape_article(client, article_url, article_name, semaphore, limit)
        )
...
async def scrape_collection(
    ...
    for link in category_links:
        ...
        category_payload = await scrape_category(
            client, category_url, category_name, semaphore, limit
        )
        if category_payload:
...
async def run_scraper(concurrency: int = 2, limit: int = 16) -> List[Collection]:
    """
    ...
    Args:
        concurrency: Maximum number of concurrent requests allowed.
        limit: Maximum number of articles to scrape.
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        print(f"Starting scraper with concurrency={concurrency} and limit={limit}....")
        ...
        for link in collection_links:
            ...
            tasks.append(
                scrape_collection(
                    client, collection_url, collection_name, semaphore, limit
                )
            )
