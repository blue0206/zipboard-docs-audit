...
async def healthcheck_handler():
    return ApiResponse(
        success=True, status_code=200, payload="The server is up and running."
    )
...
async def api_exception_handler(req: Request, error: ApiError) -> ApiResponse:
    ...
    return ApiResponse(
        success=False, status_code=error.status_code, payload=error.payload
    )
...
async def global_exception_handler(req: Request, error: Exception) -> ApiResponse:
    ...
    return ApiResponse(success=False, status_code=500, payload="Internal Server Error")
